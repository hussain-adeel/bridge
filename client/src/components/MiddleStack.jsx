...
export default function MiddleStack({ cardsOnTable, myIndex, trickWinnerIndex }) {
    ...
    const getCardHighlight = (card) =>
        card.playerIndex === trickWinnerIndex
            ? "rounded-lg ring-4 ring-amber-300 shadow-[0_0_28px_rgba(252,211,77,0.9)] animate-pulse"
            : "";
    ...
    return (
        <div className="trick-table relative mx-auto h-64 w-64">
            ...
            {playedCards.top && (
                <div className={`absolute top-4 left-1/2 -translate-x-1/2 z-20 transform ${getCardHighlight(playedCards.top)}`} style={{ transform: `rotate(${180 + getStableRotation(playedCards.top)}deg)` }}>
                    <Card suit={playedCards.top.suit} rank={playedCards.top.rank} />
                </div>
            )}
            {playedCards.right && (
                <div className={`absolute top-1/2 right-4 -translate-y-1/2 z-30 transform ${getCardHighlight(playedCards.right)}`} style={{ transform: `rotate(${-90 + getStableRotation(playedCards.right)}deg)` }}>
                    <Card suit={playedCards.right.suit} rank={playedCards.right.rank} />
                </div>
            )}
            {playedCards.bottom && (
                <div className={`absolute bottom-4 left-1/2 -translate-x-1/2 z-40 transform ${getCardHighlight(playedCards.bottom)}`} style={{ transform: `rotate(${getStableRotation(playedCards.bottom)}deg)` }}>
